#' =============================================================================
#' ex4 Main Runner - IJDS Paper
#' =============================================================================
#' 
#' Dispatcher script to run different IS methods for Example 4.
#' 
#' Usage:
#'   1. Interactive: Set `method` below and source this file
#'   2. Command line: Rscript ex4_main.R --method=Lasso
#' =============================================================================

# Configuration
ROOT <- "E:/IJDS_paper_code"  # Assumes running from IJDS_paper_code directory
RESULTS_DIR <- file.path(ROOT, "Ex4", "results")
...
METHOD_FILES <- list(
  ...
  "IS-VS"      = "ex4_IS_VS.R",
  "IS-CE"      = "ex4_IS_CE.R",
  "IS-Pareto"  = "ex4_IS_Pareto.R",
  "WAMK-SIS"   = "ex4_wamk.R",
  "SpAM"       = "ex4_SpAM.R",
  "RF-RFE"     = "ex4_randomForest.R",
  "IS-VS-GP"   = "ex4_GP.R",
  "IS-VS-RF"   = "ex4_randomForest_VS.R"
)
...
run_ex4 <- function(method, 
  ...
  method_file <- file.path(ROOT, "ex4", "methods", METHOD_FILES[[method]])
  ...
  # Save if requested
  if (save_results) {
    output_file <- file.path(RESULTS_DIR, sprintf("ex4_%s.Rdata", 
                                                   gsub("-", "_", method)))
...
#' param ... Arguments passed to run_ex4
run_all_methods <- function(methods = names(METHOD_FILES), ...) {
  ...
  for (method in methods) {
    tryCatch({
      all_results[[method]] <- run_ex4(method, ...)
    }, error = function(e) {
...
if (!interactive()) {
  ...
  run_ex4(method, n_reps = n_reps, ncores = ncores)
}

# =============================================================================
# Interactive usage example (uncomment to run)
# =============================================================================
# result <- run_ex4("Lasso", n_reps = 25, ncores = 25) # Done
# result <- run_ex4("IS-CE", n_reps = 25, ncores = 25) # Done
# result <- run_ex4("IS-Pareto", n_reps = 25, ncores = 25) # Done
# result <- run_ex4("RF-RFE", n_reps = 25, ncores = 25) # Done
# result <- run_ex4("WAMK-SIS", n_reps = 25, ncores = 25) # Done
# result <- run_ex4("SpAM", n_reps = 25, ncores = 25) # Done
result <- run_ex4("IS-VS", n_reps = 25, ncores = 25) #Done
