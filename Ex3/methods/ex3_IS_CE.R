...
#' 
#' Uses greedy Cross-Entropy based subset selection for D=10.
#'
...
run_is_ce <- function(n_reps = 25, 
  ...
  POE_est <- foreach(j = 1:n_reps, 
    ...
    for (simu in 1:n_iter) {
      ...
      # Greedy CE ordering take smallest CE subset
      h_s_all <- S_find_h_s_ce_order_greedy2(weight, sample_Z_all, S_all)
      subset <- h_s_all[[1]]
      
      index <- c(index, list(subset))
      weight_ce <- weight / sum(weight[subset])
      weight_ce_all <- c(weight_ce_all, list(weight_ce))
      
      sample_X_new <- sampling_X_new(Nm, sample_X_all, sample_Z_all, h_new_mat, combo, weight_ce, subset)
      sample_Y_new <- get_Y(sample_X_new)
      sample_Z_new <- (sample_Y_new > l)
      sample_S_new <- S_add2(sample_X_new, sample_X_all, sample_Z_all, h_new_mat, combo, weight_ce, subset)
    ...
    #     sample_S_all = sample_S_all, index = index, weight_ce_all = weight_ce_all,
    #     bandwidth = bandwidth)
    mean(est_vec)
  }
