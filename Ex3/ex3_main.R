#' =============================================================================
#' Ex3 Main Runner - IJDS Paper
#' =============================================================================
#' 
#' Dispatcher script to run different IS methods for Example 3.
#' 
#' Usage:
#'   1. Interactive: Set `method` below and source this file
#'   2. Command line: Rscript ex3_main.R --method=Lasso
#' =============================================================================

# Configuration
ROOT <- "E:/IJDS_paper_code"  # Assumes running from IJDS_paper_code directory
RESULTS_DIR <- file.path(ROOT, "Ex3", "results")
...
run_ex3 <- function(method, 
  ...
  method_file <- file.path(ROOT, "Ex3", "methods", METHOD_FILES[[method]])
  ...
  # Save if requested
  if (save_results) {
    output_file <- file.path(RESULTS_DIR, sprintf("ex3_%s.Rdata", 
                                                   gsub("-", "_", method)))
...
#' param ... Arguments passed to run_ex3
run_all_methods <- function(methods = names(METHOD_FILES), ...) {
  ...
  for (method in methods) {
    tryCatch({
      all_results[[method]] <- run_ex3(method, ...)
    }, error = function(e) {
...
if (!interactive()) {
  ...
  run_ex3(method, n_reps = n_reps, ncores = ncores)
}

# =============================================================================
# Interactive usage example (uncomment to run)
# =============================================================================
# result <- run_ex3("Lasso", n_reps = 25, ncores = 25) # Done
# result <- run_ex3("IS-CE", n_reps = 25, ncores = 25) # Done
# result <- run_ex3("IS-Pareto", n_reps = 25, ncores = 25) # Done
# result <- run_ex3("RF-RFE", n_reps = 25, ncores = 25) # Done
# result <- run_ex3("WAMK-SIS", n_reps = 25, ncores = 25) # Done
# result <- run_ex3("SpAM", n_reps = 25, ncores = 25) # Done
result <- run_ex3("IS-VS", n_reps = 25, ncores = 25) #Done
